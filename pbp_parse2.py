...
class BallGame:
    ...
    def print_row_debug(self):
        # for debug
        row = 'Date:' + self.game_status['game_date'] + ', ' +\
              'P:' + self.game_status['pitcher'] + ', ' +\
              'B:' + self.game_status['batter'] + ', ' +\
              'BSO:' + self.game_status['balls'] + '/' +\
              self.game_status['strikes'] + '/' +\
              self.game_status['outs'] + ', ' +\
              'Inn:' + self.game_status['inning']
        if self.game_status['inning_topbot'] == 0:
            row += '초, '
        else:
            row += '말, '
        row += 'Score:' + self.game_status['score_away'] + ':' +\
               self.game_status['score_home'] + ', ' +\
               'Base:' + self.game_status['runner_1b'] + '/' +\
               self.game_status['runner_2b'] + '/' +\
               self.game_status['runner_3b'] + ', ' +\
               'Event:' + self.game_status['pa_result'] + ', ' +\
               'Pitch:' + self.game_status['pitch_result'] + ', '
